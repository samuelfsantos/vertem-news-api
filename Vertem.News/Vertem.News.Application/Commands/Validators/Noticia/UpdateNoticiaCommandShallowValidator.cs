...
{
    public class UpdateNoticiaCommandShallowValidator : BaseShallowValidator<UpdateNoticiaCommand>, IShallowValidator<UpdateNoticiaCommand>
    {
        public UpdateNoticiaCommandShallowValidator()
        {
            RuleFor(x => x.Id).NotEmpty().WithMessage("O ID deve ser informado").WithErrorCode("InvalidId");
